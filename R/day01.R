...
#' A with B, then compare B with C, then C with D, and so on. Stop when
...
#'
#' @param x some data
#' @return For Part One, `f01a(x)` returns the number of depth measurements that are
#'   larger than the previous. For Part Two, `f01b(x)` returns ....
#' @export
#' @examples
...
f01a <- function(x) {
  ...
  # compute lag-1 differences between elements
  diffs <- diff(x)

  # count how many are positive (meaning depth increased)
  sum(diffs > 0)
...
#' @rdname day01
#'
#' @importFrom dplyr tibble %>% lead mutate filter pull
...
f01b <- function(x) {
  ...
  sum3 <- tibble(one = x,
                 two = lead(x, 1),
                 three = lead(x, 2)) %>%
    mutate(sum3 = .data$one + .data$two + .data$three) %>%
    filter(!is.na(.data$sum3)) %>%
    pull(.data$sum3)
